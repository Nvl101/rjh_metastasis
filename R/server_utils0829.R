...
pareto_helper <- function(metrics_df,
  ...
  # filter out skip_cols
  end_cols <- ncol(metrics_df)
  metrics_df_filtered <- metrics_df[(skip_cols+1):ncol(metrics_df)]
  # skip gene names & filters, first columns on the left
  #vars <- names(metrics_df)[4:ncol(metrics_df)] # this bit can become buggy
  vars <- names(metrics_df_filtered)
  ...
  # filter out records with null in ok_vars columns
  # to exclude from pareto results
  metrics_df_filtered <- metrics_df[complete.cases(metrics_df[, ok_vars]), ]
  ...
  pp <- Reduce('*', prefs)
  res <- psel(metrics_df_filtered, pp, top = nrow(metrics_df_filtered))
...
flag_formatter <- formatter("span",
    style = x ~ formattable::stule(color = ifelse(isin(x, c("up", "yes")), "green", "red"))
)
